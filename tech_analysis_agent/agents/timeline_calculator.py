...
BASE_YEAR = 2020  # 기준점 (Quarter 번호 0 = 2020 Q1 이전)


def quarter_to_int(q: str) -> int:
    """
    "2028 Q1" → 정수 (2028*4 + 1 = 8113)
    비교/산술 연산에 사용.
    """
    try:
...
def backcast_timeline(
    ...
    """
    시장 개화 분기를 기준으로 역산하여 각 기술의 시작/완료 분기를 결정합니다.
    ...
    Returns
    -------
    list[dict]  타임라인 초안 (start_q, target_q, lead_time 포함)
    """
    feedback = orchestrator_feedback or {}
    drop_ids = set(feedback.get("drop", []))
    shift_map = {s["tech_id"]: s["new_start_q"] for s in feedback.get("shift", [])}
    ...
    for tid in sorted_ids:
        ...
        latest_needed = boom_int - 1  # 기본: 시장 개화 1분기 전 완료
        ...
        # ⑤ 완료 분기 = latest_needed (단, 현재보다 미래여야 함)
    ...
    # ⑧ Drop된 기술은 dropped=True 로 표시하여 포함
    for tid in drop_ids:
...
def _topological_sort(tree: dict) -> list:
    """
    의존성 트리에 대해 위상 정렬(Kahn's Algorithm) 수행.
    ...
    # 레이어 낮은 순, in_degree 낮은 순으로 큐 초기화
    queue = sorted(
...
def run_timeline_calculator(state: RoadmapState) -> dict:
    ...
    sorted_ids, timeline_draft = backcast_timeline(
        dependency_tree, market_boom_q, feedback
    )
    ...
    for item in sorted(timeline_draft, key=lambda x: x.get("start_q", "9999 Q4")):
        if item.get("dropped"):
            print(f"  {item['tech_id']} | {'[DROPPED]':<35} |")
        else:
            print(
