...
──────────────────────────
Roadmap Planner Agent - Step 3: 최종 로드맵 조립 노드

역할:
1. timeline_draft 를 기반으로 Claude 에게 각 항목의
   배치 논리(justification) 생성 요청
2. Investment Strategist Agent 로 전달할 최종 planned_roadmap 완성
3. 마켓 개화 시점 도달 가능 여부 검증 (경고 메시지 포함)
"""
...
You have received a pre-calculated timeline for each technology (start_q, target_q, lead_time already determined by a backcasting algorithm).
...
2. phase_name: A meaningful label for this development stage.
   Examples: "Phase 1: Foundation R&D", "Phase 2: Process Integration", "Phase 3: System Validation"
...
- `justification` 은 반드시 **한국어 2–3 문장** 으로 작성 (영문 혼용 금지, 다만 기술 고유명사/약어는 허용: EUV, ALD, TRL 등).
- `phase_name` 은 한국어 라벨 사용. 예시:
  "1단계: 기반 R&D", "2단계: 공정 통합", "3단계: 시스템 검증", "4단계: 양산 전환"
...
Output format:
{
  "roadmap": [
    {
      "tech_id": "T01",
      "phase_name": "Phase 1: Foundation R&D",
      "justification": "..."
    }
  ]
}"""
...
"""
        ...
        print("[Roadmap Builder] Claude justification 생성 요청 중...")
        response = llm.invoke([
        ...
        for item in timeline_draft:
            ...
            roadmap_item: RoadmapItem = {
                ...
                "phase_name": (
                    ...
                    or item.get("phase_name", "Phase 1: R&D")
                ),
        ...
        # 폴백: justification 없이 timeline_draft 그대로 반환
        ...
        fallback_roadmap = [
            {
                ...
                "name": item["name"],
                "phase_name": item.get("phase_name", "Phase 1: R&D"),
                "start_q": item.get("start_q", "N/A"),
                "target_q": item.get("target_q", "N/A"),
                "prerequisites": item.get("prerequisites", []),
                "lead_time_quarters": item.get("lead_time_quarters", 0),
