...
노드 흐름:
  START
    │
    ▼
  [dependency_analyzer]   기술 트리 구성 + 레이어 할당
    │
    ▼
  [timeline_calculator]   TRL 기반 역산 → 분기별 start/target 산출
    │
    ▼
  [roadmap_builder]       Claude justification 생성 → 최종 로드맵
    │
    ▼
  END
...
def run_roadmap_planner(
    ...
    Parameters
    ----------
    tech_candidates       : Technology Analysis Agent 출력
    market_context        : {"target_market": ..., "expected_boom_quarter": ...}
    orchestrator_feedback : {"shift": [...], "drop": [...]} (선택)

    Returns
    -------
    dict  {planned_roadmap, dependency_tree, messages, error}
    """
